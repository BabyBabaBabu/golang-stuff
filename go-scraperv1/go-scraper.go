package main

import (
	"bufio"
	"fmt"
	"log"
	"os"

	"github.com/gocolly/colly"
)

var fName string
var link string
var urls []string

func main() {

	// get user supplied domain
	scanner := bufio.NewScanner(os.Stdin)
	fmt.Printf("domain: ")
	scanner.Scan()
	link = scanner.Text()
	getUrls(link)
	// saveUrls("google.txt")
	listUrls(urls)
	fmt.Println("Completed!")
}

// scrape for urls
func getUrls(link string) []string {

	c := colly.NewCollector()
	// search for all href links in 'a' tag [url/paths]
	c.OnHTML("a[href]", func(e *colly.HTMLElement) {
		links := e.Attr("href")
		urls = append(urls, links)
	})
	// print informational message
	c.OnRequest(func(r *colly.Request) {
		fmt.Println("\nVisiting: ", r.URL.String())
	})
	// visit the address/website
	c.Visit(link)

	return urls
}

// save urls to file
func saveUrls(fName string) {

	urls_ := fName
	file, err := os.OpenFile(urls_, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Fatalf("Could not create file, err :%q", err)
		return
	}
	defer file.Close()
	// write to file
	for _, url := range urls {
		if _, err := file.WriteString(url + "\n"); err != nil {
			log.Fatalf("Could not write to file, err :%q", err)
			return
		}
	}
}

// print urls to stdout
func listUrls(urls []string) {
	// loop through the slice listing found urls
	fmt.Println("\t\tFound the following URLs:")
	for _, url := range urls {
		fmt.Println(url)
	}
}
